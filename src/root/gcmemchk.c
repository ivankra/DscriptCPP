 ...
 * and does out of memory checking
 */

Mem mem;

// where to go for out of memory errors
...
//	GC::line = 0;
    ...
    else if (this == &mem)		// don't cache global mem
    {
//	PRINTF("Using global gc for size %d, file = '%s', line = %d\n", size, GC::file, GC::line);
//	GC::file = NULL;
//	GC::line = 0;
	p =  gc_get()->malloc(size);
    }
    else
    {
...
void Mem::error()
{
    ...
    longjmp(g_setjmp_buf, 1);	// return out of error condition
}
