...
void *Mem::malloc_atomic(size_t size)
{
    if (gc)			// if cached allocator
    {
//	PRINTF("Using cached gc for size %d, file = '%s', line = %d\n", size, GC::file, GC::line);
//	GC::file = NULL;
//	GC::line = 0;
	return ((GC *)gc)->malloc_atomic(size);
    }
    if (this == &mem)		// don't cache global mem
    {
//	PRINTF("Using global gc for size %d, file = '%s', line = %d\n", size, GC::file, GC::line);
//	GC::file = NULL;
//	GC::line = 0;
	return gc_get()->malloc_atomic(size);
    }
//    PRINTF("Generating cached gc for size %d, file = '%s', line = %d\n", size, GC::file, GC::line);
    gc = gc_get();
    return gc->malloc_atomic(size);
}
...
#if 0
    // EronH:  Removed to prevent 507s in low memory conditions.  This allows
    // the application to recover from errors, rather than just exiting.
    if (!p)
	mem.error();
#endif
    return p;
...
extern "C" GC *gc_get()
{
    ...
    {
	cc = gc_cache_cc;
	//PRINTF("getGC_context(): cache x%x\n", ti);
    }
    else
    {
	...
	for (ccp = gccontext; ccp < ccptop; ccp++)
	{
	    ...
	    if (cc->threadid == ti)
	    {
		//WPRINTF(L"getGC_context(): existing x%x\n", ti);
		goto Lret;
	    }
	}
